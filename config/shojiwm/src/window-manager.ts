...
export const WINDOW_STATE_RESTORE_RECT = createWindowState<ManagedWindowRect | null>(
  'restoreRect',
  {
    default: null,
  },
...
export const WINDOW_STATE_FULLSCREEN_RESTORE_RECT = createWindowState<ManagedWindowRect | null>(
  'fullscreenRestoreRect',
  {
    default: null,
  },
)
...
export const WINDOW_STATE_TILE_REORDERING = createWindowState<boolean>('tileReordering', {
  default: false,
})
...
export const WINDOW_STATE_FLOATING_RECT = createWindowState<ManagedWindowRect | null>(
  'floatingRect',
  {
    default: null,
  },
...
function hotReloadDebug(message: string, details: Record<string, unknown> = {}): void {
...
export class HybridWindowManager {
  ...
  private readonly tileabilitySubscriptionsByWindowId = new Map<string, () => void>()
  ...
  public onOutputChange(event: OutputChangeEvent) {
    const liveMonitors = new Set(
      event.outputs.filter((output) => output.enabled).map((output) => output.name),
    )
    ...
    for (const workspace of orphanedWorkspaces) {
      ...
      const wasActiveOnRemovedMonitor =
        orphanedActiveWorkspaceByMonitor.get(workspace.monitor) === workspace.index
  ...
  private trackWindowTileability(window: WaylandWindow) {
    ...
    const onChange = () => {
      ...
      const isTileable = window.isResizable.peek() && !window.isTransient.peek()
  ...
  private initializeWindowLayout(
    ...
    if (existingWorkspace) {
      return {
        ...
        restoredExistingWindow: this.restoredDuringInitialConfigure.has(window.id),
      }
    }
  ...
  public onInitialConfigure(window: WaylandWindow) {
    ...
    const { workspace, restoredExistingWindow } = this.initializeWindowLayout(window)
  ...
  public onFirstCommit(window: WaylandWindow) {
    ...
    const layoutWasDeferred = this.deferredInitialLayoutWindowIds.delete(window.id)
    const initialized = this.initializeWindowLayout(window, {
      restoreScrollIfInitiallyFloating: layoutWasDeferred,
    })
    const restoredDuringInitialConfigure = this.restoredDuringInitialConfigure.delete(window.id)
  ...
  public onWindowMove(event: WindowMoveEvent) {
    ...
    const maximizedMoveDrag =
      this.maximizedMoveDrag?.windowId === window.id ? this.maximizedMoveDrag : null
  ...
  private onFloatingWindowMove(event: WindowMoveEvent, workspace: Workspace) {
    ...
    const nextRect: ManagedWindowRect = maximizedMoveDrag
      ? this.restoreRectForMaximizedMove(event, maximizedMoveDrag.width, maximizedMoveDrag.height)
      : event.currentRect
  ...
  public cycleFocusedWindow(direction: -1 | 1) {
    const workspace = this.getCurrentWorkspace()
    if (!workspace) {
      return
    }

    const windows = workspace.listWindows()
    if (windows.length < 2) {
      return
    }
    ...
    const nextIndex =
      focusedIndex === -1
        ? direction === 1
          ? 0
          : windows.length - 1
        : (focusedIndex + direction + windows.length) % windows.length

    this.activateWindowById(windows[nextIndex].id)
  }
  ...
  private finishWorkspaceScrollGesture(event: GestureSwipeEvent) {
    ...
    workspace.startKineticScroll(
      -event.velocityX * this.workspaceGestureSpeed.workspaceScrollKineticFactor,
      () => {
        this.focusWindowAtPointerPosition(event.position ?? this.lastPointerPosition, monitor)
        this.applyWorkspaceStackPolicy(workspace)
      },
  ...
  private finishWorkspaceGesture(event: GestureSwipeEvent) {
    ...
    const shouldCommit =
      ...
      (Math.abs(event.totalY * this.workspaceGestureSpeed.workspaceSwitchFactor) >=
        gesture.distance * WORKSPACE_GESTURE_THRESHOLD_RATIO ||
        Math.abs(event.velocityY * this.workspaceGestureSpeed.workspaceSwitchVelocityFactor) >=
  ...
  private focusWindowAtPointerTarget(target: PointerMoveEvent['target'], monitorHint?: string) {
    ...
    this.currentMonitor =
      monitorHint && COMPOSITOR.output.list.includes(monitorHint) ? monitorHint : workspace.monitor
  }
  ...
  private restoreRectForMaximizedMove(
    ...
    const pointerOffsetY =
      event.source === 'modifier' ? height / 2 : Math.min(height / 2, titlebarCenterY)
  ...
  private floatingSnapLayoutFromWindows(
    ...
    for (const window of windows) {
      ...
      if (isTopSnapZone(zone)) {
        const samples = snapColumn(zone) === 'left' ? leftSplitYSamples : rightSplitYSamples
    ...
    for (const window of windows) {
      ...
      if (isTopSnapZone(zone)) {
        if (snapColumn(zone) === 'left') {
          ...
          groups.rightTopHeight = Math.max(groups.rightTopHeight, minSize.height)
        }
      } else if (isBottomSnapZone(zone)) {
        if (snapColumn(zone) === 'left') {
          groups.leftBottomHeight = Math.max(groups.leftBottomHeight, minSize.height)
        } else {
          groups.rightBottomHeight = Math.max(groups.rightBottomHeight, minSize.height)
        }
      }
    }
  ...
  private updateFloatingDragSnap(event: WindowMoveEvent) {
    ...
    const monitor =
      event.outputName && COMPOSITOR.output.list.includes(event.outputName)
        ...
        : this.currentMonitor
    const zone = monitor
      ? this.floatingSnapZoneAt(monitor, event.currentPointer.x, event.currentPointer.y)
      : null
    ...
    if (
      this.floatingSnap &&
      (this.floatingSnap.windowId !== event.window.id || this.floatingSnap.monitor !== monitor)
    ) {
...
export class Workspace {
  ...
  public addWindow(window: WaylandWindow, options: AddWindowOptions = {}): boolean {
    ...
    if (restored?.floatingRect && !this.isTiled) {
      ...
      if (restored?.tileWidth === undefined) {
        ...
        if (options.restoreScrollIfInitiallyFloating) {
          this.rememberInitialTileState(window, previousScrollOffset, previousActiveWindowId)
        }
      ...
      const initialRect = this.centeredFloatingRect(window)
      const contentRect =
        restored?.floatingRect ?? this.viewportRectToFloatingContentRect(initialRect)
  ...
  public removeWindow(window: WaylandWindow): WaylandWindow | null | undefined {
    ...
    if (index >= 0) {
      ...
      if (this.activeWindowId === window.id) {
        nextFocus =
          this.tileableWindows()[Math.min(index, this.tileableWindows().length - 1)] ?? null
  ...
  public setTiled(tiled: boolean) {
    ...
    if (tiled) {
      ...
      this.activeWindowId =
        (focusedTileableWindow ?? previousActiveWindow ?? tileable.at(0))?.id ?? null
  ...
  public applyLayout(options: LayoutOptions = {}) {
    ...
    const animationOptions =
      animate && suppressSSDRebuild && canSuppress ? MANAGED_WINDOW_ONLY_ANIMATION : undefined
    ...
    if (tileable.length === 0) {
      ...
      this.applyFloatingLayout(animationOptions, animate, options.immediateFloatingWindowIds)
      return
    }
  ...
  public focusWindowUnderPointer(window: WaylandWindow): WaylandWindow | undefined {
    if (!this.isTiled || !this.hasWindow(window) || window.state[WINDOW_STATE_MINIMIZED]()) {
      return undefined
    }

    const focused = this.focusedWindow()
    if (focused && focused.id !== window.id && this.areTransientRelatives(focused, window)) {
      return undefined
    }
  ...
  public startKineticScroll(initialVelocityX: number, onFrame?: () => void): void {
    this.stopKineticScroll()
    if (!this.isTiled || Math.abs(initialVelocityX) < WORKSPACE_KINETIC_SCROLL_MIN_VELOCITY) {
      return
    }
  ...
  public focusRelative(direction: -1 | 1) {
    ...
    const currentIndex =
      activeIndex >= 0 ? activeIndex : (fallbackIndex ?? (direction < 0 ? tileable.length : -1))
  ...
  public reclassifyWindow(window: WaylandWindow, wasTileable: boolean) {
    ...
    if (!isTileable) {
      ...
      if (this.activeWindowId === window.id) {
        const windowIndex = this.windows.findIndex((current) => current.id === window.id)
        const tileable = this.tileableWindows()
        this.activeWindowId =
          (restoredInitialActiveWindowId &&
          tileable.some((current) => current.id === restoredInitialActiveWindowId)
            ? restoredInitialActiveWindowId
    ...
    this.applyLayout({
      suppressSSDRebuild: false,
      preserveMissingActive: true,
      immediateFloatingWindowIds: !isTileable ? new Set([window.id]) : undefined,
    })
  }
  ...
  private tileableWindows(): WaylandWindow[] {
    return this.windows.filter(
      (window) => this.shouldTile(window) && !window.state[WINDOW_STATE_MINIMIZED](),
    )
  }
  ...
  private tileInsertionIndexAfterFocusedWindow(): number {
    const tileable = this.tileableWindows()
    const focused = this.focusedWindow()
    const anchor = focused && this.shouldTile(focused) ? focused : this.activeWindow(tileable)
    if (!anchor) {
      return tileable.length
    }

    const anchorIndex = tileable.findIndex((window) => window.id === anchor.id)
  ...
  private captureFloatingRect(window: WaylandWindow) {
    if (!window.state[WINDOW_STATE_FLOATING_RECT]()) {
      const rect = this.isTiled
        ? this.viewportRectToFloatingContentRect(window.state[WINDOW_STATE_RECT]())
        : window.state[WINDOW_STATE_RECT]()
  ...
  private applyFloatingLayout(
    ...
    for (const window of this.floatingWindows()) {
      ...
      const contentRect =
        ...
        this.viewportRectToFloatingContentRect(this.centeredFloatingRect(window))
      window.state[WINDOW_STATE_FLOATING_RECT].set(contentRect)
  ...
  private tileWidthForWindow(window: WaylandWindow, viewportRect: ManagedWindowRect): number {
    ...
    return clamp(
      ...
      Math.max(this.minTileWidth(window, viewportRect), this.maxTileWidth(window)),
    )
  }
  ...
  private tileContentWidth(tileable: WaylandWindow[], viewportRect: ManagedWindowRect): number {
    ...
    return (
      tileable.reduce((sum, window) => sum + this.tileWidthForWindow(window, viewportRect), 0) +
...
function managedRectContainsPoint(rect: ManagedWindowRect, x: number, y: number): boolean {
  const left = read(rect.x)
  const top = read(rect.y)
  return x >= left && x < left + read(rect.width) && y >= top && y < top + read(rect.height)
}
...
function managedRectEquals(a: ManagedWindowRect, b: ManagedWindowRect): boolean {
  return (
    read(a.x) === read(b.x) &&
    read(a.y) === read(b.y) &&
...
function snapZonesConflict(current: SnapZone | null, next: LayoutSnapZone): boolean {
...
function scheduleMinimizeAnimation(window: WaylandWindow, minimized: boolean): void {
  window.scheduleAnimation({
    channel: MINIMIZE_ANIMATION_CHANNEL,
    rect: {
      from: minimized ? { x: 0, y: 0, width: 0, height: 0 } : { x: 0, y: 200, width: 0, height: 0 },
      to: minimized ? { x: 0, y: 120, width: 0, height: 0 } : { x: 0, y: 0, width: 0, height: 0 },
      duration: OPEN_CLOSE_ANIMATION_DURATION,
      easing: minimized ? WINDOW_MINIMIZE_RECT_EASING : WINDOW_UNMINIMIZE_RECT_EASING,
      mode: 'add',
    },
    opacity: {
      from: minimized ? 1 : 0,
      to: minimized ? 0 : 1,
      duration: OPEN_CLOSE_ANIMATION_DURATION,
      easing: minimized ? WINDOW_MINIMIZE_OPACITY_EASING : WINDOW_UNMINIMIZE_OPACITY_EASING,
      mode: 'override',
    },
  })
}
...
function resetWorkspaceVisualState(window: WaylandWindow, visible: boolean): void {
  cancelWorkspaceVisualAnimation(window)
  window.state[WINDOW_STATE_WORKSPACE_VISIBLE].set(visible)
  window.state[WINDOW_STATE_WORKSPACE_OFFSET_Y].set(0)
  window.state[WINDOW_STATE_WORKSPACE_OPACITY].set(visible ? 1 : 0)
